// Definisikan class OrdersPage terlebih dahulu
class OrdersPage {
    constructor() {
        this.orders = [];
        this.filteredOrders = [];
        ...
        // Register this instance globally and with the router
    ...
    // Method statis untuk memastikan instance tersedia secara global
    static initializeGlobal() {
        if (!window.ordersPage) {
            window.ordersPage = new OrdersPage();
            // console.log('Created new OrdersPage instance globally');
        }
        return window.ordersPage;
    }
    ...
    setupEventListeners() {
        // Delay setup sedikit untuk memastikan DOM telah diperbarui
        setTimeout(() => {
            const newOrderBtn = document.getElementById('new-order-btn');
            if (newOrderBtn) {
                // console.log('Setting up new order button listener');
                ...
                applyDateFilterBtn.addEventListener('click', () => {
                    if (startDateInput && endDateInput) {
                        ...
                        if (!startDate || !endDate) {
                            this.showNotification('Mohon isi rentang tanggal', 'error');
                            return;
                        }
                        ...
                        this.filterOrders(this.dateFilter.startDate, this.dateFilter.endDate, statusFilter.value);
                    }
                });
            }
        }, 500);
    ...
    setupWebSocketListeners() {
        ...
        // Reload orders specifically when requested
        window.addEventListener('reloadOrders', async () => {
            console.log('Reloading orders due to external request');
    ...
    // Helper to apply current filters after data reloaded
    applyCurrentFilters() {
        const periodFilter = document.getElementById('period-filter');
        const statusFilter = document.getElementById('status-filter');
    ...
    /**
     * Menangani event orderUpdate dari WebSocket
     * @param {Object} data 
     */
    handleOrderUpdate(data) {
        // Hanya log action untuk tracking event
        const action = data.action || 'unknown';
        console.log(`Order update: ${action} (ID: ${data.order?.id || data.order_id || 'N/A'})`);
        
        switch (action) {
            case 'create':
                this.showNotification(`Pesanan baru #${data.order.id} telah dibuat`, 'info');
                this.loadOrders(); // Muat semua pesanan
                break;
                
            case 'update':
                if (data.order) {
                    const orderIndex = this.orders.findIndex(order => order.id === data.order.id);
                    if (orderIndex !== -1) {
                        // Update pesanan dalam array
                        this.orders[orderIndex] = data.order;
                        
                        // Jika modal detail terbuka untuk pesanan ini, perbarui data di modal
                        if (this.currentOrderDetail && this.currentOrderDetail.id === data.order.id) {
                            this.populateOrderDetails(data.order);
                        }
                        
                        // Tampilkan notifikasi
                        this.showNotification(`Pesanan #${data.order.id} telah diperbarui`, 'info');
                        
                        // Perbarui tampilan
                        this.applyCurrentFilters();
                    } else {
                        this.loadOrders(); // Pesanan tidak ditemukan, muat ulang semua
                    }
                }
                break;
            ...
            case 'status_update':
                if (data.order && data.order.id) {
                    ...
                    let statusText = statusMappings[data.order.status] || data.order.status;
                    ...
                    this.showNotification(`Status Pesanan #${data.order.id} berubah menjadi: ${statusText}`, notificationType);
                    
                    if (orderIndex !== -1) {
                        // Update pesanan dalam array
                        this.orders[orderIndex].status = data.order.status;
                        
                        // Jika modal detail terbuka untuk pesanan ini, perbarui status di modal
                        if (this.currentOrderDetail && this.currentOrderDetail.id === data.order.id) {
                            this.populateOrderDetails(data.order);
                        }
            ...
            case 'payment_update':
                if (data.order) {
                    ...
                    let message = `Pembayaran untuk Pesanan #${data.order.id}`;
                    if (data.order.payment_status === 'paid') {
                        message += ' telah lunas';
                    } else if (data.order.payment_status === 'pending') {
                        message += ' menunggu pembayaran';
                    } else {
                        message += ` status: ${data.order.payment_status}`;
                    }
                    
                    let notificationType = data.order.payment_status === 'paid' ? 'success' : 'info';
                    this.showNotification(message, notificationType);
                    
                    if (orderIndex !== -1) {
                        // Update pesanan dalam array
                        this.orders[orderIndex].payment_status = data.order.payment_status;
                        
                        // Jika modal detail terbuka untuk pesanan ini, perbarui status pembayaran di modal
                        if (this.currentOrderDetail && this.currentOrderDetail.id === data.order.id) {
                            this.populateOrderDetails(data.order);
                        }
            ...
            case 'delete':
                if (data.order_id) {
                    const orderIndex = this.orders.findIndex(order => order.id === data.order_id);
                    if (orderIndex !== -1) {
                        // Tampilkan notifikasi
                        this.showNotification(`Pesanan #${data.order_id} telah dihapus`, 'warning');
                        
                        // Hapus dari array
                        this.orders.splice(orderIndex, 1);
                        
                        // Tutup modal jika sedang menampilkan pesanan yang dihapus
                        if (this.currentOrderDetail && this.currentOrderDetail.id === data.order_id) {
            ...
            case 'served':
                if (data.order_id) {
                    this.showNotification(`Pesanan #${data.order_id} telah disajikan`, 'success');
                    this.loadOrders(); // Muat ulang pesanan
                    this.applyCurrentFilters();
                }
                break;
                
            default:
                // Aksi tidak dikenali, muat ulang semua pesanan untuk memastikan data terkini
                this.loadOrders();
                break;
        }
    }
    ...
    renderActionButtons(order) {
        ...
        // Tambahkan tombol lihat detail di awal dengan window
        buttons += `<button class="btn-view" title="Lihat Detail" onclick="window.ordersPage.viewOrderDetails(${order.id})">
            <i class="fas fa-eye"></i>
        </button>`;
        
        switch(order.status.toLowerCase()) {
            case 'pending_payment':
                buttons += `<button class="btn-process" title="Proses Pembayaran" onclick="window.ordersPage.processPayment(${order.id})">
                    <i class="fas fa-money-bill-wave"></i>
                </button>`;
                break;
            case 'paid':
                buttons += `
                    <button class="btn-start" title="Mulai Memasak" onclick="window.ordersPage.startCooking(${order.id})">
                        <i class="fas fa-utensils"></i>
                    </button>
                    <button class="btn-receipt" title="Cetak Struk" onclick="window.ordersPage.printReceipt(${order.id})">
                        <i class="fas fa-receipt"></i>
                    </button>
                `;
                break;
            case 'in_progress':
                buttons += `<button class="btn-finish" title="Selesai Memasak" onclick="window.ordersPage.finishCooking(${order.id})">
                    <i class="fas fa-check-circle"></i>
                </button>`;
                break;
            case 'ready':
                buttons += `<button class="btn-complete" title="Pesanan Selesai" onclick="window.ordersPage.completeOrder(${order.id})">
                    <i class="fas fa-check-double"></i>
                </button>`;
                break;
            case 'completed':
                buttons += `<button class="btn-receipt" title="Cetak Struk" onclick="window.ordersPage.printReceipt(${order.id})">
                    <i class="fas fa-receipt"></i>
                </button>`;
                break;
        }
    ...
    showOrderDetailsModal(order) {
        ...
        // Aktifkan modal setelah ditambahkan ke DOM
        setTimeout(() => {
    ...
    renderDetailActionButton(order) {
        ...
        switch(order.status.toLowerCase()) {
            case 'pending_payment':
                button = `<button class="btn-primary" onclick="window.ordersPage.processPayment(${order.id})">
                    <i class="fas fa-money-bill-wave"></i> Proses Pembayaran
                </button>`;
                break;
            case 'paid':
                button = `
                    <button class="btn-primary" onclick="window.ordersPage.startCooking(${order.id})">
                        <i class="fas fa-utensils"></i> Mulai Memasak
                    </button>
                    <button class="btn-receipt" onclick="window.ordersPage.printReceipt(${order.id})">
                        <i class="fas fa-receipt"></i> Cetak Struk
                    </button>
                `;
                break;
            case 'in_progress':
                button = `<button class="btn-primary" onclick="window.ordersPage.finishCooking(${order.id})">
                    <i class="fas fa-check-circle"></i> Selesai Memasak
                </button>`;
                break;
            case 'ready':
                button = `<button class="btn-primary" onclick="window.ordersPage.completeOrder(${order.id})">
                    <i class="fas fa-check-double"></i> Pesanan Selesai
                </button>`;
                break;
            case 'completed':
                button = `<button class="btn-receipt" onclick="window.ordersPage.printReceipt(${order.id})">
                    <i class="fas fa-receipt"></i> Cetak Struk
                </button>`;
                break;
            default:
                ...
                <div class="modal-body">
                    <form id="new-order-form" class="responsive-form">
                        ...
                        <div class="order-items-container">
                            ...
                            <button type="button" class="btn-add-item" onclick="window.ordersPage.addOrderItemInput()">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn-secondary" onclick="window.ordersPage.closeModal()">Batal</button>
            ...
            <div class="order-item-input" data-index="${index}">
                <div class="form-row">
                    <div class="form-group menu-select-group">
                        <label>Menu *</label>
                        <select class="menu-select" required onchange="window.ordersPage.updateItemPrice(this)">
                    ...
                    <div class="form-group quantity-group">
                        ...
                        <input type="number" class="quantity-input" min="1" value="1" required 
                               onchange="window.ordersPage.updateItemPrice(this.parentElement.parentElement)">
                    </div>
                    ...
                    ${index > 0 ? `
                        <button type="button" class="btn-remove-item" onclick="window.ordersPage.removeOrderItemInput(${index})">
...
// Register the orders page with the router - pindahkan setelah class didefinisikan
(function() {
    ...
    const registerRoute = function() {
        if (window.router) {
            ...
            window.router.addRoute('orders', async () => {
                // Create the OrdersPage instance globally if not exists
                if (!window.ordersPage) {
...
// Inisialisasi instance global dengan aman setelah class didefinisikan
(() => {
    try {
        if (!window.ordersPage) {
            // console.log('Initializing global OrdersPage instance');
            window.ordersPage = new OrdersPage();
        }
    } catch (error) {
        console.error('Error initializing global OrdersPage:', error);
    }
})(); 